...
if nargin < 2
    ...
    fprintf('   ''detector-car'' will load a car detector, ''detector-person'', ''detector-bicycle'' similarly\n');
...
DATA_DIR = 'C:\Users\Young Wu\Desktop\Dropbox\CSC420\project\code\data\';
DETECTOR_DIR = 'C:\Users\Young Wu\Desktop\Dropbox\CSC420\project\data\detectors\';

switch whatdata
    case {'loadpred'} % loads predictors
        pred = load(strcat(imname,'_results.mat'));
        data.cyclist = pred.result{1};
        data.car = pred.result{2};
        data.person = pred.result{3};
    case {'list'}
        directory = strcat(DATA_DIR,imset);
        files = dir(fullfile(directory,'\left\*.png'));
        data.ids = {files(1:end).name};
...
%         data.ids = ids;
    case {'left', 'left-plot', 'right', 'right-plot'}
        ...
        imfile = fullfile(DATA_DIR, imset, leftright, sprintf('%s.png', imname));
        im = imread(imfile);
        data.im = im;
    ...
    case {'gt-left-road','gt-left-lane'}
        type = strsplit(whatdata,'-');
        label_dir = fullfile(DATA_DIR, imset, sprintf('gt_%s', type{2})); 
        im = imread(strcat(label_dir,'\',strrep(imname,'_',strcat('_',type{3},'_')),'.png'));
        data.gt = im;
    ...
    case {'detector-car', 'detector-person', 'detector-pedestrian', 'detector-bicycle'}
